...
El HUD muestra día, fase, tiempo restante o progreso, velocidad seleccionada x0,5/x1/x2/x4 y estado de pausa independiente. Reanudar conserva la velocidad seleccionada.

### Apertura y checkout

`Open Store` queda deshabilitado con explicación si no existe checkout funcional. Durante `Open`, el último checkout no puede retirarse. Una invalidación sobrevenida muestra una acción de reparación o cierre y evita nuevas admisiones.

### Guardado

El botón de guardado manual vive en el panel de pausa. Solo se habilita en `BeforeOpen`, `Closed` o `Results` sin mutaciones pendientes; el motivo del bloqueo es visible y accionable.

### Pedidos y Management

`Process All` presenta total reservado y confirma todo-o-nada. El histórico permite cambiar entre día actual, dos días detallados anteriores, summaries individuales más antiguos y lifetime, con scroll y estados vacíos.

### Settings y cámara

El toggle de wall occlusion se oculta. Una preferencia antigua no modifica el resultado efectivo.
